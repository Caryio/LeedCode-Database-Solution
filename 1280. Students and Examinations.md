...
```sql
# Write your MySQL query statement below、
select t1.student_id, t1.student_name, t1.subject_name, ifnull(t2.attended_exams,0) as attended_exams from (select s1.student_id, s1.student_name, s2.subject_name
from Students s1 join Subjects s2 order by s1.student_id, s2.subject_name) as t1
left join
